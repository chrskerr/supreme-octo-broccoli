...
class PlanesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create plane" do
    assert_difference('Plane.count') do
      post planes_url, params: { plane: { column: @plane.column, name: @plane.name, row: @plane.row } }
    end
  ...
  test "should update plane" do
    patch plane_url(@plane), params: { plane: { column: @plane.column, name: @plane.name, row: @plane.row } }
